...
function SignupPage({ theme, onToggleTheme }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!form.name.trim() || !form.email.trim() || form.password.length < 6) {
      setError("Enter a name, a valid email, and a password with at least 6 characters.");
      return;
    }
    ...
    try {
      ...
      setError(requestError.response?.data?.error || requestError.message || "Signup failed.");
    }
  };
    ...
    >
      <div className="w-full rounded-[2rem] border border-white/20 bg-white/55 p-8 shadow-glass backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/45">
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-cyan-700 dark:text-cyan-300">Join The Platform</p>
            <h1 className="mt-3 text-3xl font-semibold text-slate-900 dark:text-white">Create your workspace</h1>
          </div>
        ...
        <p className="mt-6 text-sm text-slate-500 dark:text-slate-400">
          Already have an account? <Link to="/login" className="font-semibold text-cyan-700 dark:text-cyan-300">Sign in</Link>
        </p>
      </div>
    </motion.section>
  );
