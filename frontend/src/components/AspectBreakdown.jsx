...
function AspectBreakdown({ aspects }) {
  if (!aspects?.length) {
    return <p className="text-sm text-slate-500 dark:text-slate-400">No aspect cues found in this text.</p>;
  }
      ...
      {aspects.map((aspect, index) => (
        ...
        >
          <div className="mb-3 flex items-center justify-between gap-3">
            <strong className="text-base capitalize text-slate-900 dark:text-white">{aspect.aspect}</strong>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Score: {Number(aspect.score ?? 0).toFixed(2)}</p>
        </motion.article>
      ))}
    </div>
  );
