...
function detectSarcasm(text, sentimentResult) {
  ...
  const positiveTokenCount = tokens.filter((token) =>
    ["wow", "great", "awesome", "perfect", "amazing", "love"].includes(token),
  ).length;
  const negativeTokenCount = tokens.filter((token) =>
    NEGATIVE_CUES.includes(token),
  ).length;
  const mismatch =
    positiveTokenCount > 0 &&
    (negativeTokenCount > 0 || sentimentResult.score < -0.2);

  // New patterns for sarcasm detection
  const exaggeratedIntensity =
    tokens.some((t) => INTENSITY_MODIFIERS.has(t)) &&
    sentimentResult.score < -0.1;
  const rhetorical = /[?!]{2,}/.test(text); // Multiple punctuation marks
  const ironyMarkers =
    /yeah.*right|oh.*sure|oh.*please|how (wonderful|nice|fun)/.test(lower);
  ...
  const sarcasm = score >= 0.65 ? "High" : score >= 0.35 ? "Medium" : "Low";
