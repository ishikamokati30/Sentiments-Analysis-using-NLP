...
const MULTILINGUAL_SENTIMENT_MODEL =
...
function mapSentimentPayload(payload, model) {
  ...
  const positive =
    predictions.find((item) => /label_2|positive/i.test(item.label))?.score ||
    0;
  const neutral =
    predictions.find((item) => /label_1|neutral/i.test(item.label))?.score || 0;
  const negative =
    predictions.find((item) => /label_0|negative/i.test(item.label))?.score ||
    0;
...
function fallbackSentiment(text) {
  ...
  const positive =
    normalized > 0
      ? Math.min(0.9, 0.5 + normalized / 2)
      : Math.max(0.05, 0.25 + normalized / 3);
  const negative =
    normalized < 0
      ? Math.min(0.9, 0.5 + Math.abs(normalized) / 2)
      : Math.max(0.05, 0.25 - normalized / 3);
...
async function analyzeSentiment(text, language = "english") {
  ...
  let result =
    mapSentimentPayload(payload, model) || fallbackSentiment(normalized);
